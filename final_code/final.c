...
void left_role_2()   // left robot operation
{
  ...
  while(1){   // görüntü 0 da iken kesin duracak ayarla
              // soft 0 ken pwmlere 0 ver
               ...
               ref = speed_ar(softpot_real); // softpot deðeri balon ortadaysa 0 verir
...
void right_role() //right role operation of the robot
{
 ...
 // starting_right(3000);
    ...
    float flex_difference = 0.0;
        ...
        flex_ave = flex_read(); // artýk discrete okuyoruz
        Serial.println("flex_ave");
        Serial.println(flex_ave);
        
        
        softpot_real = soft_read_real(0);//sað soft okunuyor 0 ile
    ...
    if(softpot_dis==1)
    {
        ...
        extra_pwm_1 = 1;        // hýzlanma referansý
...
//pwm1--> sað teker
      ...
      if(flex_difference<=0){
                ...
                pwm1 =27- 3*flex_difference; //saða dogru hareket
